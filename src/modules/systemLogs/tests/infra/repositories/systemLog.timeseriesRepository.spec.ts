...
const record: SystemLogRecordFormat = [
  ...
  new SystemLogMessageProps("device's update failed"),
  SystemLogSections.VIDEO_DEVICES_CONFIG,
  'device-id',
];

function makeRepository(tdengineClient: { exec: jest.Mock }) {
  return new SystemLogRepository(tdengineClient, {
    ...
    await expect(
      repository.insert({
        data: [
          ...
          new SystemLogMessageProps('device.update.failed'),
          SystemLogSections.VIDEO_DEVICES_CONFIG,
          'device-id',
        ],
      }),
