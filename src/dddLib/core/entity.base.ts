...
export abstract class Entity<EntityValueObjects, EntityProps> {
  constructor({
    id,
    createdAt,
    updatedAt,
    props,
  }: CreateEntityProps<EntityValueObjects>) {
    this.setId(id);
  ...
  protected readonly props: EntityValueObjects;
   ...
   * @param object Entity
   */
  public equals(object?: Entity<EntityValueObjects, EntityProps>): boolean {
   ...
   * @memberof Entity
   */
  public getValueObjects(): EntityValueObjects & BaseEntityProps {
    const propsCopy = {
      id: this._id,
  ...
  public getProps(): EntityProps & BaseEntityProps {
    const flatEntityProps: any = {};
    for (const key in this.props) {
      if (this.props[key] instanceof ValueObject) {
        flatEntityProps[key] = this.props[key].unpack();
      } else flatEntityProps[key] = this.props[key];
    }
    const flatProps = {
      id: this._id,
      createdAt: this._createdAt,
    ...
    return Object.freeze(flatProps);
  }
  ...
  private validateProps(props: EntityValueObjects): void {
    const MAX_PROPS = 50;
  ...
  createValueObjectIfDefined<T, U>(
    value: T | undefined,
